...
NEW_ISBNS = [
    ("9781260460445", "Lange Q&A Radiography Examination — min qty 5 on BooksGoat"),
    ("9780990873853", "Overcoming Gravity — min qty 5 on BooksGoat"),
...
for fname in ['repricer.py', 'scanner.py']:
    ...
    for isbn, reason in NEW_ISBNS:
        if isbn in txt:
            print(f"{fname}: {isbn} already present — skipping")
            continue
    ...
    if added:
        print(f"{fname}: added {len(added)} ISBNs: {added}")
...
print("\nRun: git add repricer.py scanner.py booksgoat_enhanced.csv && git commit -m 'Full blocklist sync - 11 ISBNs' && git pull --rebase origin main && git push")
