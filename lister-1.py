...
def get_book_image(isbn13: str, isbn10: str = "", title: str = "") -> str | None:
    """
    Full 6-source image fallback chain (ported from fix_listings.py + title search).
    ...
    # 6. Google Books API — title search fallback when ISBN search finds nothing
    #    Catches niche publishers (ACI, 979x ISBNs) not indexed by ISBN
    if title:
        try:
            # Clean title for search — remove format suffixes and special chars
...
def list_books():
    ...
    for i, opp in enumerate(opportunities):
        ...
        # Full 6-source image lookup with ISBN-10 + title fallback
        isbn10 = isbn13_to_isbn10(isbn)
        cover_url = get_book_image(isbn, isbn10, title)
        if cover_url:
