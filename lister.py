...
Fixes applied:
  ...
  v2.4 — HTML email templates via email_helpers module

Run: GitHub Actions scanner.yml after scanner.py
"""

import os, csv, json, base64, time, logging, requests, sys, re
...
try:
    ...
    build_lister_success_email = None
    build_lister_failure_email = None
...
def send_alerts(listed_books):
    """Send HTML email for successfully listed books."""
    if not listed_books:
        return
    if send_html_email and build_lister_success_email:
        subject = f'[Lister] ✅ {len(listed_books)} new listings'
        html = build_lister_success_email(listed_books)
        send_html_email(subject, html)
    else:
        # Fallback if email_helpers not available
        log.warning('email_helpers not available — skipping success email')


def send_failure_alert(failed_books):
    """Send HTML email for books that failed to list."""
    if not failed_books:
        return
    if send_html_email and build_lister_failure_email:
        subject = f'[Lister] 🔴 {len(failed_books)} books FAILED to list'
        html = build_lister_failure_email(failed_books)
        send_html_email(subject, html)
    else:
        log.warning('email_helpers not available — skipping failure email')
    ...
    for i, opp in enumerate(opportunities):
        ...
        if not ensure_inventory_item(isbn, title, fmt, description, cover_url, qty, token):
            ...
            failed.append({
                ...
                'listing_price': price, 'profit': opp.get('profit', 0),
                'stage': 'inventory', 'error': 'Inventory item PUT failed',
                'action': 'Check eBay API logs',
            })
            time.sleep(0.5)
            continue
        ...
        if not offer_id:
            ...
            failed.append({
                'isbn': isbn, 'title': title,
                'listing_price': price, 'profit': opp.get('profit', 0),
                'stage': 'offer', 'error': 'POST offer failed',
                'action': 'Check for existing offer in Seller Hub',
            })
            time.sleep(0.5)
            continue
        ...
        if listing_id:
            ...
            failed.append({
                'isbn': isbn, 'title': title,
                'listing_price': price, 'profit': opp.get('profit', 0),
                'stage': 'publish', 'error': (err or '')[:60],
                'action': 'Check Seller Hub for draft listing',
            })
